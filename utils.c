...
char *ps_string_replace(char *message, const char *old_string, const char *new_string)
{
		...
		if(*p == '/') {
			*p = 0;
			...
			if(res != 0 && errno != EEXIST) {
				PS_LOG(LOG_ERR, "Error creating folder %s\n", tmp);
				return res;
			}
			*p = '/';
		}
...
int ps_pidfile_create(const char *file) {
	...
	if(pidfd < 0) {
		PS_LOG(LOG_FATAL, "Error opening/creating PID file %s, does PS have enough permissions?\n", pidfile);
		return -1;
	}
	pidf = fdopen(pidfd, "r+");
	if(pidf == NULL) {
	...
	if(flock(pidfd, LOCK_EX|LOCK_NB) < 0) {
		if(fscanf(pidf, "%d", &pid) == 1) {
			PS_LOG(LOG_FATAL, "Error locking PID file (lock held by PID %d?)\n", pid);
		} else {
			PS_LOG(LOG_FATAL, "Error locking PID file (lock held by unknown PID?)\n");
		}
		fclose(pidf);
		return -1;
	}
	...
	pid = getpid();
	if(fprintf(pidf, "%d\n", pid) < 0) {
		PS_LOG(LOG_FATAL, "Error writing PID in file, error %d (%s)\n", errno, strerror(errno));
		fclose(pidf);
		return -1;
	}
	fflush(pidf);
...
int ps_pidfile_remove(void) {
	...
	if(flock(pidfd, LOCK_UN) < 0) {
		PS_LOG(LOG_FATAL, "Error unlocking PID file\n");
		fclose(pidf);
		close(pidfd);
		return -1;
	}
