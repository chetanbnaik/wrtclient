		...
		for(i = 0; i < n; i++) {
			switch (sac->sac_info[i]) {
				...
				case SCTP_ASSOC_SUPPORTS_AUTH:
					PS_LOG(LOG_VERB, " AUTH");
					break;
				case SCTP_ASSOC_SUPPORTS_ASCONF:
					PS_LOG(LOG_VERB, " ASCONF");
					break;
				case SCTP_ASSOC_SUPPORTS_MULTIBUF:
					PS_LOG(LOG_VERB, " MULTIBUF");
					break;
				case SCTP_ASSOC_SUPPORTS_RE_CONFIG:
					PS_LOG(LOG_VERB, " RE-CONFIG");
					break;
				default:
					PS_LOG(LOG_VERB, " UNKNOWN(0x%02x)", sac->sac_info[i]);
					break;
			}
		}
	} else if(((sac->sac_state == SCTP_COMM_LOST) ||
	            (sac->sac_state == SCTP_CANT_STR_ASSOC)) && (n > 0)) {
		PS_LOG(LOG_VERB, ", ABORT =");
		for(i = 0; i < n; i++) {
			PS_LOG(LOG_VERB, " 0x%02x", sac->sac_info[i]);
		}
	}
	PS_LOG(LOG_VERB, ".\n");
...
void janus_sctp_handle_peer_address_change_event(struct sctp_paddr_change *spc) {
	...
	PS_LOG(LOG_VERB, "Peer address %s is now ", addr);
	switch (spc->spc_state) {
		case SCTP_ADDR_AVAILABLE:
			PS_LOG(LOG_VERB, "SCTP_ADDR_AVAILABLE");
			break;
		case SCTP_ADDR_UNREACHABLE:
			PS_LOG(LOG_VERB, "SCTP_ADDR_UNREACHABLE");
			break;
		case SCTP_ADDR_REMOVED:
			PS_LOG(LOG_VERB, "SCTP_ADDR_REMOVED");
			break;
		case SCTP_ADDR_ADDED:
			PS_LOG(LOG_VERB, "SCTP_ADDR_ADDED");
			break;
		case SCTP_ADDR_MADE_PRIM:
			PS_LOG(LOG_VERB, "SCTP_ADDR_MADE_PRIM");
			break;
		case SCTP_ADDR_CONFIRMED:
			PS_LOG(LOG_VERB, "SCTP_ADDR_CONFIRMED");
			break;
		default:
			PS_LOG(LOG_VERB, "UNKNOWN");
			break;
	}
	PS_LOG(LOG_VERB, " (error = 0x%08x).\n", spc->spc_error);
	return;
}

void janus_sctp_handle_adaptation_indication(struct sctp_adaptation_event *sai) {
	PS_LOG(LOG_VERB, "Adaptation indication: %x.\n", sai-> sai_adaptation_ind);
	return;
}

void janus_sctp_handle_shutdown_event(struct sctp_shutdown_event *sse) {
	PS_LOG(LOG_VERB, "Shutdown event.\n");
...
void janus_sctp_handle_stream_reset_event(janus_sctp_association *sctp, struct sctp_stream_reset_event *strrst) {
	...
	PS_LOG(LOG_VERB, "[%"SCNu64"] Stream reset event: flags = %x, ", sctp->handle_id, strrst->strreset_flags);
	if(strrst->strreset_flags & SCTP_STREAM_RESET_INCOMING_SSN) {
		if(strrst->strreset_flags & SCTP_STREAM_RESET_OUTGOING_SSN) {
			PS_LOG(LOG_VERB, "incoming/");
		}
		PS_LOG(LOG_VERB, "incoming ");
	}
	if(strrst->strreset_flags & SCTP_STREAM_RESET_OUTGOING_SSN) {
		PS_LOG(LOG_VERB, "outgoing ");
	}
	PS_LOG(LOG_VERB, "stream ids = ");
	for(i = 0; i < n; i++) {
		if(i > 0) {
			PS_LOG(LOG_VERB, ", ");
		}
		PS_LOG(LOG_VERB, "%d", strrst->strreset_stream_list[i]);
	}
	PS_LOG(LOG_VERB, ".\n");	
...
void janus_sctp_handle_remote_error_event(struct sctp_remote_error *sre) {
	...
	PS_LOG(LOG_VERB, "Remote Error (error = 0x%04x): ", sre->sre_error);
	for(i = 0; i < n; i++) {
		PS_LOG(LOG_VERB, " 0x%02x", sre-> sre_data[i]);
	}
	PS_LOG(LOG_VERB, ".\n");
	return;
}
...
void janus_sctp_handle_send_failed_event(struct sctp_send_failed_event *ssfe) {
	...
	if(ssfe->ssfe_flags & SCTP_DATA_UNSENT) {
		PS_LOG(LOG_VERB, "Unsent ");
	}
	if(ssfe->ssfe_flags & SCTP_DATA_SENT) {
		PS_LOG(LOG_VERB, "Sent ");
	}
	if(ssfe->ssfe_flags & ~(SCTP_DATA_SENT | SCTP_DATA_UNSENT)) {
		PS_LOG(LOG_VERB, "(flags = %x) ", ssfe->ssfe_flags);
	}
	PS_LOG(LOG_VERB, "message with PPID = %d, SID = %d, flags: 0x%04x due to error = 0x%08x",
	       ntohl(ssfe->ssfe_info.snd_ppid), ssfe->ssfe_info.snd_sid,
	       ssfe->ssfe_info.snd_flags, ssfe->ssfe_error);
	n = ssfe->ssfe_length - sizeof(struct sctp_send_failed_event);
	for(i = 0; i < n; i++) {
		PS_LOG(LOG_VERB, " 0x%02x", ssfe->ssfe_data[i]);
	}
	PS_LOG(LOG_VERB, ".\n");
	return;
}
...
void *janus_sctp_thread(void *data) {
	...
	if(sctp == NULL) {
		PS_LOG(LOG_ERR, "Invalid SCTP association, closing thread\n");
		g_thread_unref(g_thread_self());
		return NULL;
	}
	PS_LOG(LOG_VERB, "[%"SCNu64"] Starting thread for SCTP association\n", sctp->handle_id);
	...
	PS_LOG(LOG_VERB, "[%"SCNu64"] Leaving SCTP association thread\n", sctp->handle_id);
