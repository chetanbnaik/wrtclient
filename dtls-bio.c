...
void janus_dtls_bio_filter_set_mtu(int start_mtu) {
	if(start_mtu < 0) {
		PS_LOG(LOG_ERR, "Invalid MTU...\n");
		return;
	}
	mtu = start_mtu;
	PS_LOG(LOG_VERB, "Setting starting MTU in the DTLS BIO filter: %d\n", mtu);
}
...
int janus_dtls_bio_filter_write(BIO *bio, const char *in, int inl) {
	PS_LOG(LOG_HUGE, "janus_dtls_bio_filter_write: %p, %d\n", in, inl);
...
long janus_dtls_bio_filter_ctrl(BIO *bio, int cmd, long num, void *ptr) {
	switch(cmd) {
		...
		case BIO_CTRL_DGRAM_QUERY_MTU:
			...
			PS_LOG(LOG_HUGE, "Advertizing MTU: %d\n", mtu);
			return mtu;
		...
		default:
			PS_LOG(LOG_HUGE, "janus_dtls_bio_filter_ctrl: %d\n", cmd);
	}
	return 0;
}
